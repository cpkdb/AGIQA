#!/usr/bin/env python3
"""Clean a full dimension subpool index by prompt length and prompt-noise heuristics."""
...
def clean_records(records: Iterable[Dict], **kwargs) -> Tuple[List[Dict], Dict[str, int]]:
    ...
    for record in records:
        keep, reason = classify_prompt(record.get("prompt", ""), **kwargs)
        if keep:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Clean a full dimension subpool index by prompt length and prompt-noise heuristics.")
    ...
    outputs = clean_dimension_subpools(
